...
sums         <- read.csv("../Data/2_Intermediate/Forobs_TMF/Pixel_sum.csv",stringsAsFactors = F)
...
df   <- merge_dfs("../Data/2_Intermediate/Forobs_TMF", "Forobs_", "../Data/3_Output", "Forobs_TMF.csv")

# Making consistent (and shapefile readable )
allnames        <- names(df)
allnames        <- gsub("Forobs_", "Fobs_", allnames)
names(df)       <- allnames
...
writeOGR(shp, "../Data/3_Output", "ForobsTMF_m2", driver = "ESRI Shapefile", overwrite_layer = TRUE)
