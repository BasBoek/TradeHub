...
sums         <- read.csv("../Data/2_Intermediate/Forlos_Hansen/Pixel_sum.csv",stringsAsFactors = F)
...
df   <- merge_dfs("../Data/2_Intermediate/Forlos_Hansen", "Lossyear_", "../Data/3_Output", "Forlos_Hansen.csv")

# Making consistant (and shapefile readable )
allnames        <- names(df)
allnames        <- gsub("Lossyear_", "LossY_", allnames)
names(df)       <- allnames
...
df[,3:ncol(df)] <- round(df[,3:ncol(df)] / df$sums_x1000 * 25000000, 0) # Pixel area in m2
df[,3:ncol(df)] <- sapply(df[3:ncol(df)], as.integer)                   # Store as integer

# Merge and write to file
shp <- merge(grid, df, by="id", all.x=T)
writeOGR(shp, "../Data/3_Output", "ForLossYear_m2", driver = "ESRI Shapefile", overwrite_layer = TRUE)
